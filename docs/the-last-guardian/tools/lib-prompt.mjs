// THE LAST GUARDIAN — shared prompt vocabulary.
// Both tools/build-flow-pack.mjs (clip-level) and tools/build-scene-prompts.mjs
// (scene-level) import from here so the identity cards can never drift apart.
// Edit the constants HERE — never hand-edit generated output.
...
export const CLIP_SECONDS = 8; // Veo / Flow generation window
...
export const STYLE = "Photoreal live-action cinema, 2.39:1 anamorphic widescreen, fine 35mm grain, naturalistic skin, practical lighting, no CGI gloss.";
export const AVOID = "Avoid: on-screen text, subtitles, watermarks, HUD or UI graphics, comic-book styling, capes, masks on the hero, glossy 3D-render look, extra fingers, changing hairstyles.";

/* Still-image variants (for Omni / Imagen / Midjourney-style stills) */
export const IMG_STYLE = "Cinematic film still from a live-action science-fiction feature, 2.39:1 anamorphic widescreen, shot on 35mm, fine natural grain, photoreal skin texture with pores and sweat, practical in-frame light sources, filmic contrast, muted desaturated palette.";
export const IMG_AVOID = "No text, no captions, no subtitles, no watermark, no logo, no HUD or UI overlay, no comic-book or concept-art styling, no capes, no full-face masks, no glossy 3D-render look, no extra fingers, no duplicated limbs, no hairstyle changes.";
...
export function cameraLine(sh) {
  const move = /static|locked/i.test(sh.move) ? `locked-off, no camera movement` : sh.move;
  return `${sh.size}, ${sh.angle}, ${sh.lens}, ${move}.`;
}

export function cards(sh) {
  return (sh.chars || []).filter(Boolean).map(c => CARD[c]).filter(Boolean);
}

export function beatHint(i, n) {
  if (n === 1) return '';
  const where = i === 0
...
export function buildFlowPrompt(scene, sh, clipIndex, clipCount) {
  const who = cards(sh);
